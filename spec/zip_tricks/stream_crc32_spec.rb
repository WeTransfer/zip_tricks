...
describe ZipTricks::StreamCRC32 do
  it 'computes the CRC32 of a large binary file' do
    raw = StringIO.new(Random.new.bytes(1 * 1024 * 1024))
  ...
  it 'when computing the CRC32 from an IO only allocates one String' do
    raw = StringIO.new(Random.new.bytes(64 * 1024 * 10))
    ...
    # of 64KB so if we allocate a String for each read() - which is
    # what this spec tries to prevent - we would certainly go over
    # 90 allocations.
    expect { described_class.from_io(raw) }.to allocate_under(5).objects
  end
